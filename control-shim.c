...
static int pad_accept(void *ctx,int deck,int bank){return select_pad_bank(deck,bank,ctx,pad_mode,pad_select,pad_wait);}
static void pad_emit(void *ctx,int key,int op,int ch){rx3_dispatch_key(ctx,key,op,ch,0,0.f,0);}

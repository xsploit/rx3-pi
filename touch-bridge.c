...
int main(int argc,char**argv){
 if(argc<3)return 2;
 int fullscreen=argc>3&&!strcmp(argv[3],"--fullscreen");
 int replay=!strcmp(argv[1],"--replay");int in=replay?0:open(argv[1],O_RDONLY),out=open(argv[2],O_RDWR|O_NONBLOCK);control=open(UI_CONTROL,O_RDWR|O_NONBLOCK);
 ...
 if(!replay&&(ioctl(in,EVIOCGABS(ABS_MT_POSITION_X),&ax)||ioctl(in,EVIOCGABS(ABS_MT_POSITION_Y),&ay))){perror("touch ranges");return 1;}
 struct finger fingers[10]={0};int slot=0,source=-1,ux=0,uy=0,release=0;struct input_event e;struct pollfd p={in,POLLIN,0};
 for(;;){

...
## BiteDJ 25% fourth range

The native v1.19 range slot3 now uses25% with0.05% increments instead ofWIDE. Guarded patch-tempo25.py changes the engine table, snapshot numeric table, range indicator comparison and two original counter layout flag instructions. The GUI shim replaces the existing WIDE image in memory with generated original25% artwork; no firmware artwork is included in source. Mixer supports25%, and local exhaustive fine-bin tests cover it alongside all original ranges.

Live candidate verified ten native fader/BPM cases (five positions on each cued deck), replayed installed BiteDJ FLX6 range note60 through16/25/6/10 on both decks, and eight touch fine adjustments at25%. Other deck rate remained neutral. First screenshot caught the inherited one-decimal WIDE layout; corrected instructions0x28a438/0x28a450 keep the25label but select the hundredths layout. After restart, repeated range/fine controls passed and screenshot shows native+0.05%/-0.05% on both decks with25%labels. Both native and Mixer screenshots inspected. Final ranges10,rate0,Sync off,cued,backlight0.

Earlier temporary table-only experiment verified transport progression near0.75x/1.25x on both decks; the integrated build has not yet repeated the playback/audio test. Post-Sync pickup at25%, physical fader feel/takeover and other native layouts (including simple counter) remain pending. Do not claim complete mapping coverage from this checkpoint.

### Integrated 25% Sync and playback follow-up

On unchanged player39670, eight same-track post-Sync touch tests passed at25% across both decks (±12.5% held speed, both fine directions). Each released the native hold, reached the expected adjacent0.05% bin and preserved the other deck. Both decks then played at neutral/−25%/+25%; measured track progress followed0.75x/1.25x within the test tolerance. At+25%,40/40 distinct FLX6 DMA buffers were observed for each deck, with both headphone-channel RMS values above219. These are buffer/transport checks, not a listening or physical-fader test.

Returned both decks to cue, Sync off, rate0/range10, Cue2 off, backlight0. Saved a private Pi runtime checkpoint at /home/pompu_5/rx3-checkpoints/df210b0 containing executable, shim, start/stop scripts and MIDI bridge, with SHA256SUMS verified. This is a runtime rollback checkpoint, not a full library or SD-card backup. User direction relayed from the Engine research thread: preserve this RX3 setup and prioritize separate Engine DJ experiments over further RX3 polish.

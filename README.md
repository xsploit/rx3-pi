...
- Native Mixer panel with 16 touch sliders and two headphone-cue buttons, sharing input state with FLX6. Touch replay verified level taps, drag, cue toggle and return to waveforms; audio effects remain to be checked individually.
...
Native transport is experimental. Browser/load/play/pause were exercised through touch replay; full physical touch comfort, all controls and full state feedback are not verified. Full mixer audio-effect verification, source-less Browse escape by touch, MIDI LEDs, pad modes, shifted jogs, deck3/4 policy, reconnect handling and reproducible reboot installation remain incomplete. Browser acceleration is not ported; the encoder currently moves one native step per MIDI delta. VIEW-long/SHIFT variants remain unmapped. Scripts retain the current Pi's paths, group IDs and FLX6 card identity and require adapting to another installation.
